...
async function callGemini(prompt, model, apiKey) {
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    const r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.3, responseMimeType: "application/json", maxOutputTokens: 4000 } }) });
    if (r.ok) return JSON.parse((await r.json()).candidates?.[0]?.content?.parts?.[0]?.text || "{}");
