...
function inferDateFromUrl(url) {
  // Extract date patterns like /2026/04/15/ or /2026-04-15 from URLs
  const m = url.match(/[\/\-_](20\d{2})[\/\-_](0[1-9]|1[0-2])[\/\-_](0[1-9]|[12]\d|3[01])/);
  if (m) {
...
async function runPrompt({ label, text }, apiKey, signal) {
  try {
    ...
    for (const chunk of chunks) {
      ...
      const datePublished = inferDateFromUrl(url) || now;
...
export async function fetchLlmDiscoverySources(options = {}) {
  ...
  // Run all prompts in parallel — each is an independent search
  const results = await Promise.allSettled(
    DISCOVERY_PROMPTS.map((prompt) => runPrompt(prompt, apiKey, options.signal))
  );
