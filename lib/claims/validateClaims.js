...
const ALLOWED_CATEGORIES = [...MAIN_CATEGORIES, "uncategorised"];
...
export function validateClassification(raw = {}) {
  ...
  const main_category = ALLOWED_CATEGORIES.includes(raw.main_category)
    ? raw.main_category
    : null;

  const category_confidence =
    typeof raw.category_confidence === "number"
      ? Math.max(0, Math.min(100, Math.round(raw.category_confidence)))
      : 20;
  ...
  return {
    ...
    main_category,
    category_confidence,
    category_reason: String(raw.category_reason || "").slice(0, 500),
