...
- ai_disinformation: ONLY for AI-generated or AI-amplified synthetic narratives, fake news, coordinated inauthentic behaviour, or propaganda operations. Does NOT apply to: security research about AI systems, AI tools used defensively, vulnerability disclosures for AI products, news reporting about the AI industry, deepfake detection research, deepfake detection benchmarks, or academic analysis of deepfake technology (even if the paper's topic is deepfakes). A Google threat intelligence report about adversaries using AI to write malware is "ai_generated_malware", not "ai_disinformation". A deepfake detection paper is 0 threat tags — it is defensive research, not an attack or influence operation. Apply "ai_disinformation" only when the attack payload is false or misleading information itself (fake news, propaganda, synthetic narratives used to deceive people).
- actively_exploited: ONLY for confirmed in-the-wild exploitation of a specific vulnerability (CVE, design flaw, or named technique) by attackers against real targets. Do NOT assign for: threat actors using AI tools generally (e.g. "hackers are using AI to generate malware" — that is ai_generated_malware, not actively_exploited); reports about AI attack capabilities without a specific victim; research demonstrations without real-world exploitation confirmation.
- sensitive_data_disclosure: ONLY for attacks that extract sensitive data FROM an LLM or AI system — system prompt extraction, PII leakage via LLM memorisation, indirect prompt injection that exfiltrates context. Do NOT assign for: general data breaches unrelated to AI; stolen images used for deepfakes; social engineering that uses AI-generated content; deepfake sextortion (that is deepfake + voice_cloning if voice involved, under ai_enabled_threats).
- deepfake category guidance: sources primarily about deepfake attacks, deepfake fraud, deepfake sextortion, or AI-generated synthetic media used to harm people belong in ai_enabled_threats with the deepfake tag. deepfake detection research belongs in ai_enabled_threats or traditional_ai_threats depending on the model type, with deepfake tag only — NOT ai_disinformation.
...
- Start with "This", "It", or "These". "The" is acceptable only when followed immediately by a specific named entity, quantity, CVE ID, organisation, or technical term — never when it introduces a vague generalisation (e.g. "The growing concern", "The importance of", "The need for" are banned; "The 15,500-domain campaign", "The CVE-2026-1234 vulnerability", "The Lazarus Group technique" are fine). Start with the most specific noun phrase from the source that conveys the key fact (e.g. "Adaptive probe-based steering...", "15,500-domain AI investment scam...", "CVE-2026-44338...", "Adversarial suffix transfer...").
...
1. why_it_matters — does it start with "This", "It", or "These"? If yes, rewrite it starting with the specific technical term, CVE ID, product name, or named entity from the source.
2. ai_disinformation — is it assigned to a deepfake detection paper or defensive research? If yes, remove it.
3. actively_exploited — is it assigned because threat actors are "using AI tools" generally, without a specific victim and confirmed exploitation of a specific vulnerability? If yes, remove it.
4. sensitive_data_disclosure — is it assigned to a deepfake story or image misuse? If yes, remove it — it applies only to data extracted FROM an LLM.
