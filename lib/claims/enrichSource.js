...
function isQuotaError(status, body = "") {
  return status === 429 || body.includes("quota") || body.includes("insufficient_quota") || body.includes("RESOURCE_EXHAUSTED");
}
...
async function callOpenAICompat(source, { baseUrl, apiKey, model, label }) {
  ...
  if (!response.ok) {
    ...
    if (isQuotaError(response.status, msg)) throw Object.assign(new Error(`${label}: ${msg}`), { isQuota: true });
...
async function callGeminiModel(source, model) {
  ...
  if (!response.ok) {
    ...
    if (isQuotaError(response.status, body)) throw Object.assign(new Error(`Gemini ${model}: quota`), { isQuota: true });
...
// ── Provider rotation ────────────────────────────────────────────────────────
// Tries each provider in order; skips on quota errors, throws on other errors.
