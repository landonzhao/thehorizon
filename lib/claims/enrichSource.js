...
async function callOpenAI(source) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI enrichment failed: ${response.status} ${err?.error?.message || ""}`);
  }
...
export async function enrichSource(source) {
  ...
  if (hasOpenAI) {
    try {
      ...
      // Fall back to Gemini if OpenAI is quota-limited or unavailable
      const isQuotaError = err.message.includes("429") || err.message.includes("quota") || err.message.includes("insufficient_quota");
