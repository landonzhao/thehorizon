...
const GRAPH_CATS = Object.keys(GRAPH_COLOURS);

function buildConvergenceMatrix(convergences) {
  const nodes = GRAPH_CATS.map((c) => ({
    id:    c,
    label: GRAPH_LABELS[c],
    fill:  GRAPH_COLOURS[c],
  }));

  const edgeMap = new Map();
  for (const conv of convergences) {
    const validCats = conv.categories.filter((c) => GRAPH_CATS.includes(c));
    for (let i = 0; i < validCats.length; i++) {
      for (let j = i + 1; j < validCats.length; j++) {
        const key = [validCats[i], validCats[j]].sort().join("~~");
        if (!edgeMap.has(key)) {
          edgeMap.set(key, { from: validCats[i], to: validCats[j], weight: 0, themes: [] });
        }
        const edge = edgeMap.get(key);
        edge.weight += conv.strength;
        if (!edge.themes.includes(conv.theme)) edge.themes.push(conv.theme);
      }
    }
  }
...
export function getExpectedReportId(period, weekOffset = 0, now = new Date()) {
  const win = periodWindow(period, weekOffset, now);
  return makeReportId(period, win.week_key);
}
...
export async function generateReport({
  ...
  return {
    ...
    chart_data: {
      ...chartData,
      convergence_matrix: buildConvergenceMatrix(convergences),
    },
