...
export async function saveSnapshotToDatabase(snapshot) {
  ...
  const sourceRows = snapshot.sources.map((source) => ({
    ...
    source_text_quality:
      source.source_text_quality || "unknown",

    needs_full_text_fetch:
      source.needs_full_text_fetch || false,

    main_category: source.main_category || null,
    category_confidence:
      source.category_confidence || 0,
    category_reason:
      source.category_reason || null,
    tag_version:
      source.tag_version || null,

    content_hash: source.content_hash || null,
    clean_text_hash:
      source.clean_text_hash || null,
