...
function scoreAiSecurityRelevance(source, text) {
  // When ai_specificity_score is available (set by the classify pipeline), use it
  // as the primary signal so AI relevance drives ranking over generic severity.
  if (typeof source.ai_specificity_score === "number") {
    // Scale 0–100 specificity → 0–16 base points, then add a small category bonus.
    let score = Math.round((source.ai_specificity_score / 100) * 16);

    if (source.main_category === "agentic_ai_threats" || source.main_category === "ai_enabled_threats") score += 3;
    else if (source.main_category === "llm_threats") score += 2;
    else if (source.main_category === "traditional_ai_threats" || source.main_category === "ai_for_security") score += 1;

    // Hard cap for uncategorised: no AI relevance bonus
    if (source.main_category === "uncategorised") score = Math.min(score, 4);

    return clamp(score, 0, 20);
  }

  // Heuristic fallback when ai_specificity_score is not yet set
...
// Scores the suitability of a source for inclusion in published reports.
// Based on information density, factual specificity, and strategic value.
function scoreReportQuality(source, text) {
  let score = 0;

  // Fact density signals
  if (/cve-\d{4}-\d+/i.test(text)) score += 4;
  if (/\d+\s*(million|billion|thousand|%)/i.test(text)) score += 3;
  if (text.includes("according to") || text.includes("researchers found")) score += 2;
  if (text.includes("technical analysis") || text.includes("incident report")) score += 3;

  // Source type quality
  if (source.source_type === "research_paper") score += 5;
  if (source.source_type === "threat_intel") score += 4;
  if (source.source_type === "government_advisory") score += 3;
  if (source.source_type === "vendor_advisory") score += 2;

  // Intelligence metadata signals (set by Gemini)
  const intel = source.intelligence || {};
  if ((intel.trend_signals || []).length >= 2) score += 3;
  if ((intel.key_entities?.threat_actors || []).length > 0) score += 3;
  if ((intel.key_entities?.cves || []).length > 0) score += 2;
  if (typeof intel.horizon_relevance === "number") score += intel.horizon_relevance;

  // Penalty for low-value content
  if (text.includes("webinar") || text.includes("sponsored")) score -= 5;
  if (text.includes("press release") || text.includes("product launch")) score -= 3;
  if ((source.short_summary || "").length < 60) score -= 3;

  return clamp(score, 0, 25);
}

// Scores how early and actionable this source is for emerging threat detection.
function scoreHorizonSignal(source) {
  ...
  const reportTier = intel.report_tier;
  if (reportTier === "weekly") score += 4;
  else if (reportTier === "monthly") score += 2;
...
export function scoreSource(source) {
  ...
  // For uncategorised sources cap severity contribution so generic CVEs don't
  // outrank AI-focused sources with lower raw severity signals.
  ...
  // Report score: how useful is this source for weekly/monthly/quarterly reports?
  const report_score =
    scores.ai_security_relevance +
