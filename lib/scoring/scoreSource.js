...
function scoreAiSpecificity(source) {
  ...
  else if (cat === "traditional_ai_threats" || cat === "ai_for_security") score += 1;
...
function scoreSeverity(source, text) {
  ...
  for (const tag of HIGH_SEVERITY_TAGS) {
    if (tags.has(tag) && tag !== "actively_exploited" && tag !== "rce" && tag !== "proof_of_concept") {
      score += 2;
    }
  }
...
function scoreOperationalActionability(source, text) {
  ...
  if (tags.has("financial_sector")) score += 2;
  if (tags.has("government_sector")) score += 1;
