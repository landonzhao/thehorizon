...
function buildDeterministicTaxonomy(source) {
  ...
  // Infer rawfact_tags from existing tags + understanding taxonomy fields
  ...
  if (source.understanding?.attack_mappings) {
    for (const am of source.understanding.attack_mappings) {
      if (am.tag) rawfactTagSet.add(am.tag);
    }
  }
...
function buildUserPrompt(source) {
  ...
  const frameworkTagNames = [
    ...(u.framework_tags || []).map((ft) => ft.tag || ft.name || ft),
    ...(u.attack_mappings || []).map((am) => `${am.tag}(ATT&CK:${am.framework_ref})`),
  ].filter(Boolean).join(", ");
