...
function penalties(source) {
  ...
  // Unknown type with no taxonomy tags across all three fields
  if (
    source.source_type === "unknown" &&
    (source.understanding?.framework_tags?.length ?? 0) === 0 &&
    (source.understanding?.attack_mappings?.length ?? 0) === 0 &&
    (source.understanding?.governance_tags?.length ?? 0) === 0
  ) pen += 10;
