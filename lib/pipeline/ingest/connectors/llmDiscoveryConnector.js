...
// Articles with an inferred date older than 48h are not eligible for the daily report
// (they were discovered today but are historical content, not breaking news).
const DAILY_ELIGIBILITY_WINDOW_MS = 48 * 60 * 60 * 1000;
...
async function runPrompt({ label, text }, apiKey, signal) {
  try {
    ...
    for (const chunk of chunks) {
      ...
      // date_published is always now so the source passes the daily window filter.
      // date_published_actual carries the inferred historical date (null if unknown).
      const isRecent = !inferredDate ||
        (Date.now() - new Date(inferredDate).getTime()) < DAILY_ELIGIBILITY_WINDOW_MS;
      ...
      sources.push(
        normalizeSource({
          ...
          collection_metadata: {
            ...
            date_confidence: inferredDate ? "estimated" : "low",
            date_discovered: now,
            eligible_for_daily_report: isRecent,
          },
        })
      );
    }
