...
function buildDeterministicTaxonomy(source) {
  ...
  // Attack vectors from framework tags and attack_mappings tag names
  const attackVectors = [];
  ...
  // attack_mappings is a richer source — use tactic-normalized tag names
  for (const am of (u.attack_mappings || [])) {
    const tag = (am.tag || "").toLowerCase().replace(/-/g, "_");
    if (VALID_ATTACK_VECTORS.has(tag)) attackVectors.push(tag);
  }
...
function buildUserPrompt(source) {
  ...
  const frameworkTagList = [
    ...(u.framework_tags || []).slice(0, 5).map((ft) => `${ft.tag} (${ft.framework || ""})`),
    ...(u.attack_mappings || []).slice(0, 4).map((am) => `${am.tag} (ATT&CK:${am.framework_ref})`),
    ...(u.governance_tags || []).slice(0, 2).map((gt) => `${gt.tag} (NIST)`),
  ]
    .join(", ");
