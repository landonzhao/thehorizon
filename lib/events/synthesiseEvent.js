...
async function callOpenAI(prompt, { baseUrl, apiKey, model, label }) {
  for (let attempt = 0; attempt <= 2; attempt++) {
    ...
    if (response.ok) {
      ...
      recordTokens({ step: STEP, provider: label, prompt_tokens: data.usage?.prompt_tokens ?? 0, completion_tokens: data.usage?.completion_tokens ?? 0 });
