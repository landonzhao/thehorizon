...
const CLASSIFICATION_VERSION = "classify-v3.0";
const DELETE_THRESHOLD = 10;
const TIER_CORE = 40;
const TIER_ADJACENT = 20;

// Inter-LLM-call delay: Groq free tier is 30 RPM → need ≥2s between calls.
// Use 2.5s to stay safely under the limit without being too slow.
const LLM_INTER_CALL_MS = 2500;

function shouldUseLLM(source, useLLM = true) {
  if (!useLLM) return false;
  const hasKey = !!(process.env.OPENAI_API_KEY || process.env.GROQ_API_KEY || process.env.GEMINI_API_KEY);
...
function buildUpdate({ tags, main_category, category_confidence, category_reason,
                       ...
                       short_summary, analyst_brief, intelligence, claims, llm_used }) {
  const update = {
    tags,
    main_category,
    category_confidence,
...
async function classifyOne(source, useLLM = true) {
  let llm_used = false;
  let tags, main_category, category_confidence, category_reason;
  let ai_specificity_score, ai_specificity_reason;
  let short_summary = null, analyst_brief = null, intelligence = null, claims = null;

  if (shouldUseLLM(source, useLLM)) {
    try {
...
export async function classifyStoredSources({ start, end, limit = 1000, onlyUnclassified = true, useLLM = true } = {}) {
  let query = supabase
    .from("sources")
    .select("*")
  ...
  // tag_version is set by both rule-based and LLM classify — it's the canonical
  // "classified" marker. claim_extraction_status is LLM-only and not suitable here.
  ...
  for (const source of sources || []) {
    try {
      const { tags, main_category, category_confidence, category_reason,
              ai_specificity_score, ai_specificity_reason,
              short_summary, analyst_brief, intelligence, claims, llm_used } = await classifyOne(source, useLLM);
