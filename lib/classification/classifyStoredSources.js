...
export async function classifyStoredSources({ start, end, limit = 1000, onlyUnclassified = true } = {}) {
  let query = supabase
    .from("sources")
    .select("*")
  ...
  for (const source of sources || []) {
    try {
      const { tags, main_category, category_confidence, category_reason,
              ai_specificity_score, ai_specificity_reason,
              short_summary, analyst_brief, intelligence, claims, llm_used } = await classifyOne(source);
