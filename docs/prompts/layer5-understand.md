# Layer 5 — Source Taxonomy + LLM Understanding Prompt

## Purpose

Tag every source against the controlled framework taxonomy, assign `source_type`, and suggest `category_candidates`. Layer 5 does NOT assign the final `main_category` — that is Layer 6's job.

Key principle: **understand first, then tag**. The LLM reads and summarises the source before assigning any taxonomy tags. Tags are assigned only when the source substantively discusses the risk, technique, vulnerability, or governance issue — not because a keyword appears.
...
- ecosystem_market_signal — Ecosystem/market shifts: adoption trends, platform integrations, tooling changes
- societal_harm_signal — Confirmed societal harms from AI-enabled abuse: deepfake fraud, disinformation impact
- governance_organizational_response — How organisations adapt to AI cyber risks: policies, workflow changes
...
If the source is about governance, policy, or defensive capabilities without a specific offensive
technique, suggest no category_candidate or suggest unclear_or_adjacent.
...
**Understand first, tag second.** Steps 1 and 2 (understand + source_type) run before framework tagging. The LLM must have a clear picture of what the source is about before assigning framework tags. This prevents keyword-driven over-tagging.

**Controlled registry.** The taxonomy registry from `lib/config/taxonomyRegistry.js` is injected into the system prompt at runtime (via `buildTaxonomyContextForPrompt()`). All tags are validated post-processing — tags not in the registry are silently dropped.

**category_candidates vs main_category.** Layer 5 suggests candidates with confidence scores; Layer 6 (`classifyCategory.js`) picks the winner deterministically. This separation lets Layer 5 remain purely focused on understanding the source content.

**System prompt caching.** The system prompt includes the full taxonomy registry (~2KB). It is built once and cached in module scope — all sources in a batch use the same system prompt, avoiding repeated registry expansion.

**Entity registry hint.** The prompt explicitly lists known AI systems (MCP, LangChain, Copilot, etc.) to ensure the LLM infers correct category context even when the source doesn't use the words "LLM" or "agent."

**Fallback behavior.** `deterministicFallback()` uses keyword matching on title + text to suggest a single low-confidence category_candidate. It returns empty `framework_tags`. The existing `main_category` from the DB is preserved by Layer 6 classification (not overwritten) when Layer 5 ran in fallback mode.

**Token budget per source:** ~2,200 tokens input (system prompt with registry + source content) + ~400 tokens output ≈ $0.00039/source at gpt-4o-mini pricing.
